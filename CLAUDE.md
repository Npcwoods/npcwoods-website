...
- **Kitchen HTML pixels:** condition, city, health, and search-safe files must not contain GTM, GA4, Google Ads, `/tracking.js`, or Meta pixel. The dining-room plugin installs the site pixel. Homepage PHP is the exception (its own delayed pixels). `/pay` is the other exception. `deploy.py` refuses those markers — strip the file, do not weaken the tool.
- **Homepage:** live `/` is `page-npcwoods-home.php` on Twenty Twenty-Four, forced by `npcwoods-force-php-templates.php`. Map: `HOMEPAGE.md`. Done check is `https://npcwoods.com/` with no `?`. If you see Gutenberg / `wp-site-blocks`, the forcer died.
...
`python3 tests/guardian/guardian.py` — read-only sweep of live pages: forbidden words, empty shells, broken links, noindex on paid LPs. Kitchen HTML still must have no GTM/GA/Ads/tracking.js/Meta (see STOP). The dining-room plugin may inject the site pixel on live HTML; do not copy that back into kitchen files. Report: `content-output/guardian-reports/latest.md`. Refresh page list with `tests/guardian/build_manifest.py` after adding pages. Schedule install (needs Chris's yes): `zsh tests/guardian/install.sh`. Run with `~/Desktop/Chris-HQ/gads-env/bin/python` for Playwright spot-checks (system python lacks playwright).
