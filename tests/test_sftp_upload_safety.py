...
DEPLOY_CHANGED_SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "deploy-changed-pages.py"
...
def load_deploy_changed_module():
    spec = importlib.util.spec_from_file_location("deploy_changed_pages", DEPLOY_CHANGED_SCRIPT)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
class DeployChangedPagesSafetyTest(unittest.TestCase):
    def setUp(self):
        self.module = load_deploy_changed_module()

    def test_default_mode_is_dry_run_and_does_not_load_credentials(self):
        with mock.patch.object(self.module, "load_env") as load_env:
            output = io.StringIO()
            with redirect_stdout(output):
                exit_code = self.module.main(["deploy-changed-pages.py"])
    ...
    def test_execute_requires_confirmation_phrase(self):
        with mock.patch.object(self.module, "load_env") as load_env:
            exit_code = self.module.main(["deploy-changed-pages.py", "--execute"])
